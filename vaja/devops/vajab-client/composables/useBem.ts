...
type Key<Prefix extends string, ModifierKey extends string> = `${Prefix}--${
  | ModifierKey
  | string}`
...
export const useBem = <ModifierKey extends string, Prefix extends string>(
  ...
  modifiers:
    ...
    | Ref<Record<ModifierKey, boolean>>
    | (() => Record<ModifierKey, boolean>)
) => {
  if (__DEV__ && !prefix) {
  ...
  const modifiersList = computed(() =>
    isFunction(modifiers) ? modifiers() : unref(modifiers)
  )
  ...
  const computedBemClassesObject = computed(() => {
    return Object.entries(unref(modifiersList)).reduce(
      (classesObj: Record<string, boolean>, [modifierName, value]) => {
        if (value) {
          classesObj[`${prefix}--${kebab(modifierName)}`] = true
        }
        return classesObj
      },
      {}
  ...
  const computedBemClassesArray = computed(() =>
    Object.keys(computedBemClassesObject.value)
  )

  const computedBemClassesString = computed(() =>
    computedBemClassesArray.value.join(' ')
  )
    ...
    {},
    {
      ...
      getOwnPropertyDescriptor(_, key) {
        return Reflect.getOwnPropertyDescriptor(
          computedBemClassesObject.value,
          key
        )
      },
