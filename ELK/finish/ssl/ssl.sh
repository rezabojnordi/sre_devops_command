...
openssl req -new -x509 -sha256 -key root-ca-key.pem -subj "/C=CA/ST=GREENWEB/L=TEHRAN/O=ORG/OU=UNIT/CN=CA" -out root-ca.pem -days 3650
# Admin cert
openssl genrsa -out admin-key-temp.pem 4096
openssl pkcs8 -inform PEM -outform PEM -in admin-key-temp.pem -topk8 -nocrypt -v1 PBE-SHA1-3DES -out admin-key.pem
openssl req -new -key admin-key.pem -subj "/C=CA/ST=GREENWEB/L=TEHRAN/O=ORG/OU=UNIT/CN=admin" -out admin.csr
...
openssl req -new -key node1-key.pem -subj "/C=CA/ST=GREENWEB/L=TEHRAN/O=ORG/OU=UNIT/CN=opensearch-node1" -out node1.csr
...
openssl req -new -key node2-key.pem -subj "/C=CA/ST=GREENWEB/L=TEHRAN/O=ORG/OU=UNIT/CN=opensearch-node2" -out node2.csr
...
openssl req -new -key node3-key.pem -subj "/C=CA/ST=GREENWEB/L=TEHRAN/O=ORG/OU=UNIT/CN=opensearch-node3" -out node3.csr
